...
procedure SaveMemoryStreamToFile(const FileName: string; const MemStream: TMemoryStream);
var
  hFile: THandle;
...
begin
  ...
  hFile := CreateFile(
    PChar(FileName),
    GENERIC_WRITE,
    0,                              // эксклюзивная блокировка на время записи
    nil,
    CREATE_ALWAYS,
    FILE_FLAG_WRITE_THROUGH or FILE_ATTRIBUTE_NORMAL,
    0
  );
  ...
  try
    ...
    CloseHandle(hFile);            // благодаря флагу WRITE_THROUGH данные уже на диске
  end;
end;
...
var INI: TINIFile;
    S,FN,LOG,F_PATH,EXP_STATR,FP_TMP: String;
    i,j,t: integer;
...
//    FileStream: TFileStream;
    ...
    sAnswer, sSourceFile, sTmpFile: String;
...
procedure CloseLog(bAnsw: boolean);
  ...
  if (Regime>0) and (Regime<10) then
    begin
    SaveLog;
    WriteLn(LF,'end EXPORT('+DateTimeToStr(now)+')'+LR_);
    try
    CloseFile(LF);
    finally;
    end;
...
procedure CloseFileSleep(const sERR: String);
  begin
//  if Assigned(FileStream) then FileStream.Free;
  if RetryCount < MaxRetries then
    ...
    if length(sERR)=0 then
      ...
      CloseFromError('Файл "'+sSourceFile+'" не был звписан!',sSourceFile,'Файл не передан!'+chr(10)+'Ошибка: '+sERR);
    exit
    end;
  end;
...
FP_TMP := INI.ReadString('FILE','EXP_TMP',ExtractFilePath(FN));
...
if ParamCount=3 then
  begin
  ORAConnection.Params.Values['DataBase']:=ParamStr(1);
  ORAConnection.Params.Values['User_Name']:=ParamStr(2);
...
//    LogWrite('PSW := myCrypt('''+PSW+''',coDecrypt,'''+litter+''','+PswLen+');');
//    LogWrite('PSW = '+PSW);
    except on E: Exception do
      begin
      CloseFromError('Ошибка шифрования:'#13#10+E.Message,'','Ошибка шифрования');
      exit;
      end;
    end;
...
try
  ORAConnection.Connected:=True
    ...
    CloseFromError
      (
      LR_+'DBName='+ORAConnection.Params.Values['DataBase']+
      LR_+'USER='+ORAConnection.Params.Values['User_Name']+
...
if not ORAConnection.Connected then
  ...
  while not V.Eof do
    ...
    sTmpFile:=FP_TMP+ExtractFileName(sSourceFile);

    repeat
      try
        //FileStream := TFileStream.Create(sTmpFile, fmCreate {or fmShareExclusive});
        try
          // Копируем данные из MemoryStream в файл
          //FileStream.CopyFrom(MemoryStream, MemoryStream.Size);
          SaveMemoryStreamToFile(sTmpFile, MemoryStream);
          bSuccess:=True;
          finally
          //FileStream.Free;
          if bSuccess then
            begin
            if FileExists(sSourceFile) then DeleteFile(sSourceFile);
            bSuccess:=RenameFile(sTmpFile, sSourceFile);
            end
            else
            Inc(RetryCount);
          end;
        except on E: Exception{EFOpenError} do
          CloseFileSleep(E.Message);
        end;
